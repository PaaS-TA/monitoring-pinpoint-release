...
common_init() {
  ...
  sed '/ start /d' $CATALINA_HOME/bin/startup.sh > $CATALINA_HOME/bin/startup_pinpoint_web.sh
  echo 'export CATALINA_PID=pinpoint_web.pid' >> $CATALINA_HOME/bin/startup_pinpoint_web.sh
  echo 'exec "$PRGDIR"/"$EXECUTABLE" start "$@"' >> $CATALINA_HOME/bin/startup_pinpoint_web.sh
  chmod 755 $CATALINA_HOME/bin/startup_pinpoint_web.sh

  #CREATE PINPOINT SHUTDOWN
  sed '/ stop /d' $CATALINA_HOME/bin/shutdown.sh > $CATALINA_HOME/bin/shutdown_pinpoint_web.sh
  echo 'export CATALINA_PID=pinpoint_web.pid' >> $CATALINA_HOME/bin/shutdown_pinpoint_web.sh
  echo 'exec "$PRGDIR"/"$EXECUTABLE" stop "$@"' >> $CATALINA_HOME/bin/shutdown_pinpoint_web.sh
  #SET CHMOD
  chmod 755 $CATALINA_HOME/bin/shutdown_pinpoint_web.sh
}
...
common_start() {
  ...
  if [ -d $CATALINA_HOME/webapps/ROOT ]; then
   ...
   echo "PASS :: CATALINA ROOT is not exist ..."
   #exit 1
  fi
